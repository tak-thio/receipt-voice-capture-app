...
impl<T: UserEvent> Wry<T> {
  ...
  fn init(event_loop: EventLoop<Message<T>>) -> Result<Self> {
    ...
    let context = Context {
      ...
      // PATCH (mobile-only vendor fork): iOS/Android always ship a system WebView, while the
      // startup probe wry::webview_version() -> NSBundle bundleWithIdentifier(@"com.apple.WebKit")
      // over-releases and hard-traps (SIGTRAP) on iOS before it can return Err. Putting cfg! on the
      // left of || short-circuits the probe so it is never called on mobile. Desktop is unchanged.
      webview_runtime_installed: cfg!(any(target_os = "ios", target_os = "android"))
