...
モバイルでは保存先が **アプリ専用データ領域**(`app_data_dir/receipt-sessions`)に解決されます(`src-tauri/src/commands/mod.rs` の `resolve_storage_root`)。設定画面の「保存先ルート」はデスクトップ専用で、モバイルでは非表示・空送信になります。
