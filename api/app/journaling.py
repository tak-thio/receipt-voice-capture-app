...
receipt-app keyed suggestions on the email From/subject (Gmail-centric). Mobile
receipts have no From header, so here the account suggestion comes from the
**partner's own history** (the most-used account title for that 取引先 on this
顧問先). Everything is scoped to receipt.client_id; isolation is also enforced
by RLS. Partner alias learning (vendor string -> partner) is kept.
"""
...
from .models import Partner, PartnerAlias, Receipt
...
async def _account_from_history(
    ...
    """Most-used account title among this partner's already-journalized receipts."""
    if not partner_id:
        return None
    rows = list(
        await db.scalars(
...
async def suggest(db: AsyncSession, receipt: Receipt) -> dict:
    """Suggest {partner_id, account_title_id, sub_account_id} for a receipt."""
...
async def learn_partner(db: AsyncSession, receipt: Receipt, partner_id: UUID | None) -> None:
    """Learn vendor-string -> partner on journalize (client-scoped alias upsert)."""
