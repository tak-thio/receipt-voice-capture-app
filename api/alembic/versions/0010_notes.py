...
def upgrade() -> None:
    # Idempotent: on a FRESH db 0001's create_all already made `notes` and the
    # receipts.note_ids column. Skip the schema parts if present; always (re)apply RLS.
    ...
    if "note_ids" not in {c["name"] for c in insp.get_columns("receipts")}:
        op.add_column(
            "receipts",
            sa.Column(
                "note_ids",
